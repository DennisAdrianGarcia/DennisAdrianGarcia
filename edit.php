...
if( isset($_POST['update']))
{
    ...
    if(empty($iso) || empty($name) || empty($nicename) || empty($iso3) || empty($numcode) || empty($phonecode) ){
        ...
        if(empty($phonecode)){
            echo "<font color='red'> PhonCode field is empty. </font><br/>";
        }
        echo "<br/><a href='javascript:self.history.back();'>Go Back<a/>"; 

    } else {

        $result = mysqli_query($mysqli, "UPDATE country set id='$id', iso='$iso', name='$name', nicename='$nicename', iso3='$iso3', numcode='$numcode', phonecode='$phonecode'  WHERE id=$id");
